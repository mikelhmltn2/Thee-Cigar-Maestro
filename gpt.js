document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildEnhancedContext(userInput) {
    ...
    if (data.cigars && data.cigars.length > 0) {
      if (lowerInput.includes('maduro') || lowerInput.includes('dark')) {
        ...
        if (maduroCigars.length > 0) {
          context += `\n\nAvailable Maduro cigars: ${maduroCigars
            .slice(0, 3)
            .map(c => c.name)
            .join(', ')}`;
        }
      ...
      if (lowerInput.includes('connecticut') || lowerInput.includes('mild')) {
        ...
        if (connecticutCigars.length > 0) {
          context += `\n\nAvailable Connecticut cigars: ${connecticutCigars
            .slice(0, 3)
            .map(c => c.name)
            .join(', ')}`;
        }
    ...
    if (data.emotional && data.emotional.ritualFlows) {
      if (
        lowerInput.includes('ritual') ||
        lowerInput.includes('mood') ||
  ...
  function showError(message) {
    ...
    errorDiv.style.cssText =
      'position:fixed;top:20px;right:20px;background:rgba(220,20,60,0.9);color:white;padding:10px;border-radius:4px;z-index:9999;';
  ...
  function showSuccess(message) {
    ...
    successDiv.style.cssText =
      'position:fixed;top:20px;right:20px;background:rgba(0,128,0,0.9);color:white;padding:10px;border-radius:4px;z-index:9999;';
  ...
  function generateLocalResponse(input) {
    ...
    if (lowerInput.includes('maduro')) {
      ...
      return `Maduro wrappers offer rich, dark flavors with notes of chocolate and espresso. ${
        maduroCigars.length > 0
          ? `I recommend trying: ${maduroCigars[0].name} - ${maduroCigars[0].flavor}`
    ...
    if (lowerInput.includes('connecticut')) {
      ...
      return `Connecticut wrappers are mild and creamy, perfect for beginners or morning smokes. ${
        ctCigars.length > 0
          ? `Try: ${ctCigars[0].name} - ${ctCigars[0].flavor}`
    ...
    if (lowerInput.includes('learn') || lowerInput.includes('education')) {
      ...
      return `I can help you learn about cigars! ${
        tracks.length > 0
          ? `Available courses include: ${tracks.map(t => t.title).join(', ')}`
          : 'Topics include wrapper identification, flavor profiles, and pairing principles.'
      }`;
    }
    ...
    if (lowerInput.includes('ritual') || lowerInput.includes('relax')) {
      ...
      return `Cigar rituals enhance the experience through mindfulness. ${
        flowNames.length > 0
          ? `Try our ${flowNames[0]} ritual for a perfect session.`
          : 'Focus on the moment, the flavors, and your surroundings.'
      }`;
    }
  ...
  assistantBtn.style.cssText =
    'position:fixed;bottom:20px;right:20px;z-index:3;background:#5c4033;color:#f0e6d2;padding:10px 14px;border:none;border-radius:4px;cursor:pointer;font-weight:bold;';
  ...
  chatBox.style.cssText =
    'position:fixed;bottom:80px;right:20px;width:350px;background:#1e1e1e;color:#f0e6d2;border:1px solid #444;border-radius:8px;padding:1rem;z-index:999;display:none;max-height:70vh;overflow-y:auto;';
  ...
  conversationDiv.style.cssText =
    'max-height:200px;overflow-y:auto;margin-bottom:10px;padding:5px;background:#2c2c2c;border-radius:4px;';
  ...
  inputArea.style.cssText =
    'width:100%;height:60px;margin-bottom:8px;background:#2c2c2c;border:1px solid #666;color:#fff;padding:5px;resize:vertical;border-radius:4px;';
  ...
  sendBtn.style.cssText =
    'flex:1;padding:8px;background:#c69c6d;border:none;cursor:pointer;font-weight:bold;border-radius:4px;';

  const clearBtn = document.createElement('button');
  clearBtn.textContent = 'Clear';
  clearBtn.style.cssText =
    'padding:8px 12px;background:#5c4033;border:none;cursor:pointer;color:#f0e6d2;border-radius:4px;';

  const closeBtn = document.createElement('button');
  closeBtn.textContent = '×';
  closeBtn.style.cssText =
    'position:absolute;top:5px;right:5px;background:transparent;border:none;color:#f0e6d2;cursor:pointer;font-size:18px;';
  ...
  function checkDataAndInitialize() {
    ...
    if (data && !conversationHistory.length) {
      setTimeout(() => {
        addToConversation(
          'Welcome to Thee Cigar Maestro! I have access to all cigar data, pairings, educational content, and ritual guidance. What would you like to explore?'
        );
      }, 2000);
    }
  }
