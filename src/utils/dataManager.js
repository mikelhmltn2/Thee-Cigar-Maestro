...
class DataManager {
    ...
    this.dataSchemas.set('cigars', {
      ...
      items: {
        ...
        properties: {
          ...
          wrapper: {
            type: 'string',
            enum: ['Maduro', 'Connecticut', 'Habano', 'Natural', 'Oscuro', 'Candela'],
          },
  ...
  async loadAllData(options = {}) {
    const { showProgress = true, useCache = true, forceRefresh = false, concurrent = 3 } = options;
    ...
    try {
      ...
      const sources = Array.from(this.dataSources.entries()).sort(([, a], [, b]) => {
      ...
      await storageManager.setLocal('lastDataLoad', {
        ...
        dataKeys: Object.keys(this.data).filter(
          key =>
            this.data[key] &&
  ...
  async validateAllData() {
    const validationPromises = Array.from(this.dataSchemas.entries()).map(([key, schema]) =>
      this.validateData(key, this.data[key], schema)
    );
      ...
      for (const [prop, propSchema] of Object.entries(schema.properties)) {
        if (prop in item) {
          ...
          if (
            ...
            throw new Error(
              `${path}.${prop} should be at least ${propSchema.minLength} characters`
            );
          }
        }
      }
