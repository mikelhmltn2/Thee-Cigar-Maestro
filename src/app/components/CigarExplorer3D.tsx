...
function Cigar({ cigar, isSelected, onClick }: any) {
  ...
  return (
    <group position={cigar.position}>
      ...
      {/* Cigar Band */}
      <Cylinder args={[0.16, 0.16, 0.3, 16]} position={[-0.8, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
        <meshStandardMaterial color='#d4af37' roughness={0.3} metalness={0.7} />
      </Cylinder>

      {/* Cigar Tip */}
      <Cylinder args={[0.05, 0.12, 0.3, 8]} position={[1.2, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
...
const CigarExplorer3D = () => {
  ...
  const selectedCigarData = selectedCigar ? cigarData.find(c => c.id === selectedCigar) : null;

  return (
    <div className='relative w-full h-full'>
      {/* 3D Canvas */}
      <Canvas camera={{ position: [8, 5, 8], fov: 60 }} shadows className='w-full h-full'>
        <Environment />
        ...
        {filteredCigars.map(cigar => (
          <Cigar
            ...
            onClick={() => setSelectedCigar(selectedCigar === cigar.id ? null : cigar.id)}
          />
        ))}
          ...
          <div className='card-luxury'>
            ...
            <div className='space-y-4'>
              ...
              <div className='flex space-x-3 pt-4'>
                <button className='btn-primary flex-1 text-sm py-2'>Add to Cart</button>
                <button className='btn-secondary flex-1 text-sm py-2'>Save to Humidor</button>
              </div>
            </div>
          </div>
        </motion.div>
