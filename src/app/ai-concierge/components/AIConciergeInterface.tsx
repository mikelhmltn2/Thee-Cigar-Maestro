...
const AIConciergeInterface = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: 'assistant',
      content:
        "Welcome! I'm your personal AI sommelier. I can help you find the perfect cigar based on your preferences, mood, or occasion. What would you like to explore today?",
      timestamp: new Date(),
    },
  ]);
  ...
  const generateAIResponse = async (
    userMessage: string
  ): Promise<{ content: string; recommendations?: CigarRecommendation[]; intent: string }> => {
    // Simulate API delay
    ...
    if (intent === 'recommendation') {
      ...
      return {
        content:
          "Based on your preferences, I've found some excellent options for you. These cigars offer exceptional quality and should align perfectly with what you're looking for. Would you like me to explain more about any of these recommendations?",
        recommendations,
        intent,
      };
    }
    ...
    if (intent === 'pairing') {
      return {
        content:
          'Excellent question! Pairing cigars with drinks is an art. For whiskey, I recommend medium to full-bodied cigars with earthy or spicy notes. The robust flavors complement each other beautifully. Cuban cigars like Montecristo or Nicaraguan options like Padrón work wonderfully. Would you like specific recommendations for your whiskey preference?',
        intent,
      };
    }
    ...
    return {
      content:
        "I'm here to help you discover amazing cigars! Whether you're looking for recommendations, pairing advice, or learning about cigar culture, I have the knowledge of master blenders and decades of expertise. What specific aspect of cigars interests you most?",
      intent: 'general',
    };
  };
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                    message.role === 'user' ? 'bg-background-accent' : 'bg-gradient-gold'
                  }`}
                >
                  {message.role === 'user' ? (
                ...
                <div className={`space-y-2 ${message.role === 'user' ? 'text-right' : ''}`}>
                  <div
                    className={`rounded-luxury-lg p-4 ${
                      message.role === 'user'
