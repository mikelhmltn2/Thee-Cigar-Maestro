...
class AIRecommendationEngine {
    ...
    try {
      ...
      const scoredCandidates = await Promise.all(
        candidates.map(async cigar => {
          ...
          const hybridScore =
            contentScore * 0.3 + collaborativeScore * 0.25 + contextScore * 0.2 + aiScore * 0.25;
      ...
      const topRecommendations = scoredCandidates.sort((a, b) => b.score - a.score).slice(0, count);
   ...
   */
  private calculateContentBasedScore(cigar: CigarProfile, preferences: UserPreferences): number {
    let score = 0;
    let totalWeight = 0;
    ...
    // Price range
    if (cigar.price >= preferences.priceRange.min && cigar.price <= preferences.priceRange.max) {
   ...
   */
  private async calculateCollaborativeScore(userId: string, cigarId: string): Promise<number> {
   ...
   */
  private calculateContextualScore(cigar: CigarProfile, context?: RecommendationContext): number {
    ...
    if (context.budget) {
      const budgetScore =
        cigar.price <= context.budget ? 1 : cigar.price <= context.budget * 1.2 ? 0.7 : 0.3;
      score += budgetScore;
      factors++;
    }
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a master cigar sommelier. Rate how well this cigar matches the user preferences on a scale of 0-1, considering all factors.',
          },
          {
            role: 'user',
            content: prompt,
  ...
  private async generateReasoning(
    ...
    if (cigar.premiumLevel === 'ultra-premium' || cigar.premiumLevel === 'exclusive') {
      reasons.push(
        `${cigar.premiumLevel.charAt(0).toUpperCase() + cigar.premiumLevel.slice(1)} selection for discerning aficionados`
      );
    }
  ...
  private matchExperienceLevel(cigar: CigarProfile, level: string): string | null {
    ...
    return matches[level]?.[cigar.strength] || matches[level]?.[cigar.premiumLevel] || null;
  }
  ...
  private async getCandidateCigars(
    ...
    this.cigarDatabase.forEach(cigar => {
      // Price filter
      if (
        cigar.price < preferences.priceRange.min ||
      ...
      // Avoidance filter
      if (
        preferences.avoidances.some(
          avoid => cigar.flavorProfile.includes(avoid) || cigar.wrapper.includes(avoid)
        )
      ) {
        return;
      }
