...
function generateReport() {
  ...
  console.info(`Success Rate: ${((results.loadTest.successfulRequests / results.loadTest.totalRequests) * 100).toFixed(1)}%`);
...
async function runPerformanceTests() {
  ...
  try {
    ...
    if (desktopAudit) {
      ...
      const avgScore = (desktopAudit.performance + desktopAudit.accessibility + 
                       desktopAudit.bestPractices + desktopAudit.seo) / 4;
      results.summary.overall = avgScore >= 90 ? 'excellent' : 
                               avgScore >= 80 ? 'good' : 
                               avgScore >= 70 ? 'fair' : 'poor';
    }
